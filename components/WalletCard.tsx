...
export default function WalletCard({ isDesktop = false, onFundWallet }: WalletCardProps) {
  ...
  const getChainInfo = (chainId: number) => {
    switch (chainId) {
      case 13939:
        return { name: 'Reef Mainnet', color: '#00D4AA', isReef: true }
      case 13940:
        return { name: 'Reef Testnet', color: '#00D4AA', isReef: true }
      case 1:
        return { name: 'Ethereum', color: '#627EEA', isReef: false }
      case 137:
        return { name: 'Polygon', color: '#8247E5', isReef: false }
      case 10:
        return { name: 'Optimism', color: '#FF0420', isReef: false }
      case 42161:
        return { name: 'Arbitrum', color: '#28A0F0', isReef: false }
      case 8453:
        return { name: 'Base', color: '#0052FF', isReef: false }
      case 11155111:
        return { name: 'Sepolia', color: '#627EEA', isReef: false }
      default:
        return { name: `Chain ${chainId}`, color: '#666666', isReef: false }
    }
  }
      ...
      <div className="text-2xl font-medium mb-1">
        {isConnected && balance ? (
          <div className="flex items-center gap-2">
            <span>{formatBalance(balance.value)} {balance.symbol}</span>
            {getChainInfo(chainId).isReef && (
              <span className="text-xs px-2 py-1 rounded-full" style={{ backgroundColor: '#00D4AA', color: 'white' }}>
                REEF
              </span>
            )}
          </div>
