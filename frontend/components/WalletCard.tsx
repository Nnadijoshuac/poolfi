...
import { isReefChain, getReefChainInfo } from '@/lib/reef-config'
...
export default function WalletCard({ isDesktop = false, onFundWallet }: WalletCardProps) {
  ...
  const getChainInfo = (chainId: number) => {
    if (isReefChain(chainId)) {
      const reefInfo = getReefChainInfo(chainId)
      return { 
        name: reefInfo?.name || `Reef Chain ${chainId}`, 
        color: '#00D4AA', 
        isReef: true,
        isTestnet: reefInfo?.id === 13940 // Reef Testnet
      }
    }
    
    switch (chainId) {
      case 1:
        return { name: 'Ethereum', color: '#627EEA', isReef: false, isTestnet: false }
      case 137:
        return { name: 'Polygon', color: '#8247E5', isReef: false, isTestnet: false }
      case 10:
        return { name: 'Optimism', color: '#FF0420', isReef: false, isTestnet: false }
      case 42161:
        return { name: 'Arbitrum', color: '#28A0F0', isReef: false, isTestnet: false }
      case 8453:
        return { name: 'Base', color: '#0052FF', isReef: false, isTestnet: false }
      case 11155111:
        return { name: 'Sepolia', color: '#627EEA', isReef: false, isTestnet: true }
      default:
        return { name: `Chain ${chainId}`, color: '#666666', isReef: false, isTestnet: false }
    }
  }
      ...
      <div className="text-2xl font-medium mb-1">
        {isConnected && balance ? (
          <div className="flex items-center gap-2">
            <span>{formatBalance(balance.value)} REEF</span>
          </div>
        ) : (
          '0.00 REEF'
        )}
